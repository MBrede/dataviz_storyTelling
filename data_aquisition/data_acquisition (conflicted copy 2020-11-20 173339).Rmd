...
german_meta <- german_stories %>% 
  ...
  map_dfr(~nth(.,2) %>% 
            select(publish_date, guid, title, url))

merge_simple_triplet <- function(mat1,mat2){
  mat1$dimnames$cols
  
  out <- simple_triplet_matrix(i = c(mat1$i,mat2$i + max(mat1$i)),
                               )
  
}

german_words <- german_stories %>%  
  .[which(map_lgl(.,~length(.)>1))] %>% 
