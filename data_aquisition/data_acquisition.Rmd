...
opts_chunk$set(echo = FALSE,message = F, fig.align = 'center', eval = F)
...
swe_sources <- read_csv('data/sweden.csv') %>% # Sources of Swedish news
  filter(language == 'sv') %>% 
  select(media_id, url, name, stories_per_day)
...
```{r}
merge_simple_triplets <- function(mat1,mat2){ #combines two triplets into one
...
combine_all_triplets <- function(triplets){ # iterative wrapper to combine all sets
  out <-  triplets[[1]]
...
german_doc_term <- german_stories %>% 
  map(~first(.)) %>% 
  combine_all_triplets() %T>%
  write_rds('data/german_dtm.rds',
...
swedish_doc_term <- swedish_stories %>% 
  map(~first(.)) %>% 
  combine_all_triplets() %T>%
  write_rds('data/swedish_dtm.rds',
            compress = 'gz') #save for later
```


## pre-processing for the text vizualisations
...
remove_stoppers <- function(dtm, language = 'german'){
  ...
  log_idx <- str_detect(dtm$dimnames$cols,'[^[:alpha:]-]', negate = T) & 
             ...
             !(dtm$dimnames$cols %in% stopwords(language))
  dtm[,log_idx]
}
