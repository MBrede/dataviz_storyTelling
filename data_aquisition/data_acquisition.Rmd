...
get_stories <- function(time_range,query,media_ids){
  base_url <- 'https://api.mediacloud.org/api/v2/stories_public/'# kern-Api-Adresse
  
  ## Doc-Term-Mat
  query_string <- glue('?q=text:', query) # thematische Eingrenzung
  
  url <- glue(base_url,'word_matrix', query_string, '&key=', mc_key) # als doc-term matrix
  
  resp <-
    POST(
      glue(
        url,
        '&fq=publish_day:', time_range,# zeitliche Eingrenzung
        '&rows=1000&stopword_length=long' # bis zu 1000 Artikel, lange stopwords entfernen
      ),
      body = jsonlite::toJSON(list(fq = list(media_id = media_ids))) # id_body
    )
  
  cont <- content(resp) # body auslesen
  
  story_ids <- names(cont$word_matrix) # ids extrahieren
